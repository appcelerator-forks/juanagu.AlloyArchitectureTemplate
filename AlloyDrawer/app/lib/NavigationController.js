...
NavigationController.prototype.close = function(numWindows) {
	...
	if ((this.windowStack.length > 1) && (numWindows > 1)) {
		...
		for (var j = 1; j < numWindows; j++) {
			...
			this.windowStack[i].fireEvent('set.to.close', {
	...
	Ti.API.debug("End Home. Stack: " + this.windowStack.map(function(v) {
		return v.title;
	}));
};
...
NavigationController.prototype.open = function(/*Ti.UI.Window*/windowToOpen) {
	...
	windowToOpen.addEventListener('close', function() {
		if (that.windowStack.length > 1)// don't pop the last Window, which is the base one
		{
			...
			if (this.toClose) {
				...
				(that.navGroup) ? that.navGroup.closeWindow(this.toClose, {
					animated : true
				}) : this.toClose.close({
					animated : true
				});
			...
			Ti.API.debug("End event 'close'. Stack: " + that.windowStack.map(function(v) {
				return v.title;
			}));
	...
	Ti.API.debug("End Open. Stack: " + this.windowStack.map(function(v) {
		return v.title;
	}));
};
...
NavigationController.prototype.openFromHome = function(windowToOpen) {
	Ti.API.debug("openFromHome function.");
	if (this.windowStack.length == 1)
	...
	Ti.API.debug("End openFromHome. Stack: " + this.windowStack.map(function(v) {
		return v.title;
	}));
};
